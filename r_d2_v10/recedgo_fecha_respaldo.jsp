...
<form name="form" method="post" action="recedgo_fecha.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&programa=<%=programa_jv%>&perfil=<%=perfil%>">
      ...
      <table width="854" height="45" border="0" align="center" cellpadding="2">
          ...
          <td width="311" height="41">
            <table width="270" height="0%" border="0" cellpadding="2">
              <tr>
                ...
                <input name="txtf_date1" type="text" size="20" value="<%= day1 %>/<%=res%>/<%= year %>" onKeyUp="putFecha(this.form)" onKeyPress="return handleEnter(this, event)" readonly="true"/></td>
              </tr>
          </table>          </td>
		...
		%>
           ...
           %>
            ...
            <tr>
              <td width="8000" height="52" class="style2">NOMBRE DEL PACIENTE:
                <label> </label>
                <span class="style5">
                <label> </label>
        ...
        <tr>
          <td height="219" colspan="3" class="style4"><table width="875" border="0" align="center" cellpadding="2">
            ...
            <tr>
              ...
              <td colspan="2" class="style11"><input type="text" name="txtf_sur1" size="5" value="<%=sur1_jv%>"  onchange="setSur(this.form)" onKeyPress="return validar(event)"/></td>
