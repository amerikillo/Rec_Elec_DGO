...
financiamiento[0]="";
origen[0]="";
int pos=0,tam2=0,x1=0,cont3=0,ban=0,posf=0,ban2=0,cant2_jv=0,cv_med2=0,cv_med3=0,cont1=0,cont5=0,cont6=0,sur2=0,sol2=0,mtotal=0,total2=0,total=0,flag=0,customerIds=0,cant_pasti_ampu=0,cant_surpas_amp=0,ttcant_surpas_amp=0,ttcant_surpas_amp2=0,mtotalotsur=0,mtotalotsur2=0,resto22_jv=0;
...
int cant2_o1_jv=0,cant2_o2_jv=0,mtotalot=0,mtotal_col=0;
...
if(but.equals("Imprimir"))
     {
	  ...
	  %>
	  <script>
	  self.location='recibo_col.jsp?folio=<%=foliore2_jv%>&folio2=<%=customerIds%>&receta5=RC&encar=<%=ela2_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>';
...
<form name="form" method="post" action="recedgo_col.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&cause=<%=cause_jv%>&encar=<%=ela2_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&part=<%=part_jv%>">
<table width="957" height="562" border="3" align="center" cellpadding="2">
  <tr>
    <td width="725" height="156"><table width="801" border="0" align="center" cellpadding="2">
      <tr>
        <td width="164"><img src="logo_salud.jpg" width="114" height="78" /></td>
        <td width="396"><div align="center"><span class="style7">SERVICIO DE SALUD DE DURANGO<br />
        RECETA COLECTIVA </span></div></td>
        <td width="221"><div align="right"><img src="logo_dgo.jpg" width="126" height="76" /></div></td>
      </tr>
    </table>
              ...
              <tr>
                <td width="221" height="100%" class="style2">FECHA:<input name="txtf_date" type="text" class="style13" onKeyPress="return handleEnter(this, event)" value="<%=day1%>/<%=res%>/<%= year %>" size="1" readonly="true" style="visibility:hidden"/>
                  ...
                  <input name="txtf_t1" type="text" onKeyPress="return handleEnter(this, event)" onKeyUp="putDays()" value="<%=t1_jv%>" size="1" maxlength="2"/>
                    ...
                    <strong>                    /</strong>
                    <input name="txtf_t3" type="text" size="2" maxlength="4" onKeyUp="putYearss2(this.form)" value="2012<%//=t3_jv%>" onKeyPress="return handleEnter(this, event)" /><br />
...
<option value="YESOS Y CURACIONES">YESOS Y CURACIONES</option>
            ...
            <tr>
              <td colspan="2" class="style11">Ingrese descripci&oacute;n: 
                <input type="text" name="txtf_descmed" size="20" value="<%=desdmed_jv%>" onKeyPress="return handleEnter(this, event)" />                <input name="Submit" type="submit" class="but" value="Por Descripción" onClick="getfocus()"/></td>
           ...
           %>
            ...
            <tr>
             ...
             <td colspan="4" class="style11"><span class="style2"><span class="Estilo10">TOTAL EXISTENCIAS:</span><span class="Estilo7"> <%=sumatotal%>&nbsp;</span></span>
            ...
            <tr>
              ...
              <td colspan="2" align="center" class="style11"><div align="left"><span class="style2">CANT. SUR</span></div></td>
