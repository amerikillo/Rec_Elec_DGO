	...
	if(but.equals("Guardar"))
     {
		  ...
		  }else
		{
			 ...
			 %>
		  <script>
		  alert('CAMPO VACIO, Ingrese una Observación');
		  </script>
		  <%
	 ...
	 rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+fechanew+"', '%d/%m/%Y')"); 
	   ...
	   stmt1.execute("insert into modificacion values ('"+clave_jv+"','"+descrip_jv+"','-','-','"+exist_jv+"','"+origen_jv+"','"+ap2_jv+"','"+total+"','"+fechanew+"','"+am22_jv+"','ENTRADA MANUAL AL INVENTARIO','"+obs_jv+"',current_timestamp,'-','-','-','-','-','-',0)");	   
...
<table width="666" height="346" border="3" align="center" cellpadding="2" bgcolor="#FFFFFF">
  <tr>
    ...
    <td width="650"><form id="form" name="form" method="post" action="carga_clave_modi.jsp?usuario=<%=am22_jv%>">
      <table width="650" height="323" border="0" align="center" cellpadding="2">
        <tr>
          ...
          <td height="49" colspan="2" bgcolor="#FFFFFF" class="logo style1"><div align="center">Entrada por Ajuste  al Inventario </div></td>
