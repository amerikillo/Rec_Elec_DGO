...
String cv_dgo_jv="",cv_uni_jv="",cv_mpio_jv="",part_jv="",id_med_jv="",encarser_jv="";
try{
  ...
  med1_jv=request.getParameter("txtf_med1");
	 ...
	 rset1= stmt.executeQuery("select * from clave_med where descrip='"+descripmed_jv+"' ");
	  ...
	  cont3++;
          ...
          rset1= stmt.executeQuery("select clave from pasti_ampu where clave='"+clave2_jv+"' and cant>'1' ");
...
if(but.equals("Imprimir"))
     {
	  ...
	  %>
	  <script>
	  self.location='recibo_col.jsp?folio=<%=foliore2_jv%>&folio2=<%=customerIds%>&receta5=RC&encar=<%=ela2_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&encarser=<%=encarser_jv%>&encarser=<%=encarser_jv%>';
...
<form name="form" method="post" action="recedgo_col.jsp?uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&cause=<%=cause_jv%>&encar=<%=ela2_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&servicio=<%=servicio_jv%>&part=<%=part_jv%>&encarser=<%=encarser_jv%>">
              ...
              <tr>
                <td width="221" height="100%" class="style2">FECHA:<input name="txtf_date" type="text" class="style13" onKeyPress="return handleEnter(this, event)" value="<%=day1%>/<%=res%>/<%= year %>" size="1" readonly="true" style="visibility:hidden"/>
                  ...
                  <input name="txtf_t1" type="text" onKeyPress="return handleEnter(this, event)" onKeyUp="putDays()" value="<%=t1_jv%>" size="1" maxlength="2"/>
                    ...
                    <strong>                    /</strong>
                    <input name="txtf_t3" type="text" size="2" maxlength="4" onKeyUp="putYearss2(this.form)" value="2013<%//=t3_jv%>" onKeyPress="return handleEnter(this, event)" /><br />
