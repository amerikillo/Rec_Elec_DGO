...
rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+date2_jv+"', '%d/%m/%Y')"); 
	...
	{
 rset__001=stmt__001.executeQuery("select clave,descrip,sum(cant_sol),sum(cant_sur),present from receta_colectiva where unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+org_jv+"'  and tipo_receta='rc' group by clave order by (clave+0)");
...
<table width="1048" border="0" align="center" cellpadding="2">
  <tr>
    ...
    <td height="63" colspan="2" align="center" valign="bottom" nowrap="nowrap" bgcolor="#FFFFFF" id="logo"><div align="center">
	  ...
	  REPORTE GLOBAL DE RECETA COLECTIVA<br />
...
<table width="20%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr bgcolor="#CCFF99">
      ...
      <a href="reportes_val_rc.jsp" class="style76"><span class="Estilo1">Regresar a Menú</span></a></span>&nbsp;
 ...
 <tr>
    ...
    <td colspan="2" valign="top"><form action="reporte_glob_rc_val.jsp" method="post" name="form" onSubmit="javascript:return ValidateAbas(this)">
	...
	%>
			...
			rset_1=stmt__001.executeQuery("select sum(cant_sol),sum(cant_sur) from receta_colectiva where unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+org_jv+"' and tipo_receta='rc'");
			while(rset_1.next()) 
                  {
...
<map name="Map2" id="Map2">
  <area shape="rect" coords="6,5,33,31" href="repor_receta4.jsp?f1=<%=date11_jv%>&f2=<%=date22_jv%>&boton=Show ALL&unidad=<%=llamo_cat_jv%>&org=<%=org_jv%>" />
</map>
</body>
</html>
