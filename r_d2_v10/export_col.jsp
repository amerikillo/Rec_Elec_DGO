...
response.setHeader("Content-Disposition","attachment; filename=CONSUMO DIARIO RECETA COLECTIVA.xls");
%>
<head>
...
<p>
  <%
	...
	if (date1_jv==null && date2_jv==null){
 	rset__001=stmt__001.executeQuery("select clave, descrip, sum(cant_sur) from receta_colectiva group by clave order by (clave+0)");
	rset__002=stmt__002.executeQuery("select sum(cant_sur) from receta_colectiva");
	 }
	 else {
	 rset__001=stmt__001.executeQuery("select clave, descrip, sum(cant_sur) from receta_colectiva where fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='2' group by clave order by (clave+0)");
	rset__002=stmt__002.executeQuery("select sum(cant_sur) from receta_colectiva where fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='2' ");
 	}
